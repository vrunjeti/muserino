...
export default class Muse extends React.Component {
  ...
  /**
   * Plays a sample progression generated by Muse
   */
  playMuse() {
    const tempo = 180
    const loopCount = 4
    ...
    const progression = M.generate_progression(4)
    let progression2 = progression.map(notes => M.get_chord(notes, M.note_number('A2')))
    let progression3 = progression2.map(chord => Array(8).fill([[]]))

    for (let i = 0; i < progression3.length; i++) {
      progression3[i][0] = progression2[i].map(n => [n, 80, 4])
    }
