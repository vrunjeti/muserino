...
if __name__ == '__main__':
    ...
    for i in range(128):
        for note in verse_chords[i][0] if verse_chords[i] else []:
            to_play.append((time, note, verse_chords[i][1], verse_chords[i][2]*seconds_per_beat*4, 0))
        for note in verse_rhythm_chords[i][0] if verse_rhythm_chords[i] else []:
            to_play.append((time, note, verse_rhythm_chords[i][1], ((0.167 if i%2 == 0 else 0.083) if swing else 0.125)*seconds_per_beat*4, 1))
        for event in [(verse_arp[i], 1),
            ...
            if event[0]:
                if swing and event[0][2] == 0.125:
                    ...
                    duration = event[0][2]
                to_play.append((time, event[0][0], event[0][1], duration*seconds_per_beat*4, event[1]))
        time += ((0.667 if i%2==0 else 0.333) if swing else 0.5)*seconds_per_beat
    # Play chorus
    for i in range(128):
        for note in chorus_chords[i][0] if chorus_chords[i] else []:
            to_play.append((time, note, chorus_chords[i][1], chorus_chords[i][2]*seconds_per_beat*4, 0))
    ...
    for note in verse_chords[0][0]:
        to_play.append((time, note, verse_chords[0][1], verse_chords[0][2]*seconds_per_beat*4, 0))
    for event in [(verse_arp[0], 1),
        ...
        if event[0]:
            to_play.append((time, event[0][0], event[0][1], seconds_per_beat*4, event[1]))


    # Do the actual playing
    current_time = 0
    for note in to_play:
        if note[0] != current_time:
            time.sleep(note[0] - current_time)
            current_time = note[0]
        play(*note[1:])
